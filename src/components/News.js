 ...
 const capitalized = (string) => {
  ...
  const updateNews = async()=>{
    ...
    const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}
                 &apiKey=6adf85b3f06d4767ae9df8ec9f35ff0f&page=${page}&pageSize=${props.pageSize}`;
  ...
  useEffect(() => {
    document.title = `${capitalized(props.category)} NewsDaily`;
    updateNews();
  }, []);
  ...
  // const handlePeriviousClick = async () => {
  //   setPage(page+ 1) ;
  //   updateNews();
  // };
  const fetchMoreData = async()=>{
    ...
    const url = `https://newsapi.org/v2/top-headlines?country=${props.country}&category=${props.category}
    &apiKey=6adf85b3f06d4767ae9df8ec9f35ff0f&page=${page+1}&pageSize=${props.pageSize}`;
    setPage(page+1);
    let data = await fetch(url);
    let parsedData = await data.json();
    console.log(parsedData, "data running");
    ...
    return (
     <>
        <h1 className="text-center" style={{ margin: " 35px 0px ", marginTop:"90px"}}>
          Daily News Top {capitalized(props.category)} Headlines
        </h1>
        {loading && <Spinner/>}
        <InfiniteScroll
